...
# 激活所需的 Conda 环境
# set +u
# source /home/zirui.song/miniconda3/etc/profile.d/conda.sh
...
# Stage1 Training Configuration (7-Dimensional Rating Learning)

# Description: The first stage of training, learning detailed 7-dimensional ratings

# - Task Mode: stage1

# - Output: 7-dimensional ratings (BRF, VEQ-Clarity, VEQ-Realism, VEQ-Aesthetic, VEQ-Text, CNS-Edit, CNS-Set)

# - Dataset: CBM Standard Split Dataset

# - Training Strategy: LoRA + 7 rating heads

# - Image Compression: Current image + original input image, other images compressed into collage images
...
if [[ "${NUM_GPUS}" -ne 2 ]]; then
  echo "[ERROR] This script is used for 2-card training, currently detected${NUM_GPUS} 卡: ${CUDA_VISIBLE_DEVICES}" >&2
  exit 1
fi
...
# PyTorch 内存分配优化
# 注意：expandable_segments 在某些PyTorch版本中可能导致问题，已禁用
...
run_name="stage1-Qwen2b-$(date +%Y%m%d_%H%M%S)"
...
echo "  - 检查点保存: 每${save_steps}步保存一次 (最多保留2个，与评估同步)"
echo "========================================="
...
echo ""
echo "使用 DeepSpeed 启动：2 卡 ZeRO-2（双卡训练）"
