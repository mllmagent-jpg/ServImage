...
if python -c "import torch; exit(0 if torch.cuda.is_available() else 1)" 2>/dev/null; then
    echo "✓ CUDA detected and available"
    USE_BF16="--bf16"
    USE_DEEPSPEED="--deepspeed ${deepspeed_config}"
    CUDA_AVAILABLE=true
else
    echo ""
    echo "========================================="
    echo "⚠️  CUDA 不可用 - 无法启动GPU训练"
    echo "========================================="
    echo ""
    echo "问题：PyTorch无法初始化CUDA驱动"
    echo "原因：RunPod容器GPU配置异常"
    echo ""
    echo "解决方案："
    echo "  1. 在RunPod控制面板中重启此Pod"
    echo "  2. 重启后重新运行此脚本"
    echo ""
    echo "详细信息："
    echo "  - 查看: CUDA_ISSUE_RESOLUTION.md"
    echo "  - 诊断: bash scripts/check_cuda_env.sh"
    echo ""
    echo "如果重启后仍有问题，请联系RunPod支持。"
    echo "========================================="
    echo ""
    exit 1
fi
