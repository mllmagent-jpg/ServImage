...
# 激活所需的 Conda 环境
# 使用环境变量 CONDA_ROOT 指定 conda 安装路径，默认为 ~/miniconda3
set +u
CONDA_ROOT=${CONDA_ROOT:-"${HOME}/miniconda3"}
if [ -f "${CONDA_ROOT}/etc/profile.d/conda.sh" ]; then
  source "${CONDA_ROOT}/etc/profile.d/conda.sh"
  conda activate servimage
  echo "[Stage2] 使用Conda环境: servimage"
  echo "[Stage2] Python路径: $(which python)"
  echo "[Stage2] Python版本: $(python --version)"
else
  echo "[Stage2] 未找到 conda.sh，请确保已激活正确的 Python 环境"
  echo "[Stage2] Python路径: $(which python)"
  echo "[Stage2] Python版本: $(python --version)"
fi
set -u
...
# ==============================
# Stage2 训练配置（基于Stage1权重+新LoRA+base分类头）
# 说明：新的第二阶段训练设计
#       - 任务模式：stage2 (第二阶段训练)
#       - 加载：Stage1的checkpoint（base model + LoRA权重）
#       - 冻结：Stage1的所有权重（包括LoRA）- 完全冻结！
#       - 移除：Stage1的CBMQualityDecoder（7维质量评估头）
#       - 添加：base模型的SingleTaskQualityDecoder（二分类头）
#       - 新LoRA：在Stage1 LoRA基础上再添加新的LoRA层（双层LoRA）
#       - 训练：新LoRA + 新quality_decoder
#       - 输出：Accept/Reject (0/1标签)
#       - 数据集：CBM标准拆分数据集
#       - 训练轮数：2 epochs（用户要求）
#       - GPU数：4卡（用户要求）
# ==============================
...
NNODES=${NNODES:-1}
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-"0,1,2,3"}  # 默认使用4卡
...
export STAGE2_LORA_VISION_R=${STAGE2_LORA_VISION_R:-16}                  # LoRA rank
...
export STAGE2_LORA_PROJECTOR_R=${STAGE2_LORA_PROJECTOR_R:-16}                  # LoRA rank
...
# Language Model LoRA
export STAGE2_LORA_LLM_ENABLE=${STAGE2_LORA_LLM_ENABLE:-1}               # 1=启用, 0=禁用
export STAGE2_LORA_LLM_R=${STAGE2_LORA_LLM_R:-16}                         # LoRA rank
...
# Stage1 模型路径（默认使用checkpoint-3040）
stage1_model_path=${STAGE1_MODEL_PATH:-"output/ServImage_Model-think4b/checkpoint-3040"}

if [[ ! -d "${stage1_model_path}" ]]; then
  echo "[ERROR] Stage1 模型路径不存在: ${stage1_model_path}" >&2
  echo "请检查路径或通过 STAGE1_MODEL_PATH 环境变量指定正确的 Stage1 checkpoint 路径" >&2
  exit 1
fi
...
# Stage2 特定配置
lr=${LR:-4e-5}  # Stage2 使用较小学习率（在LoRA基础上再加LoRA）
batch_size=${BATCH_SIZE:-2}  # 4卡训练，每卡batch size=2
grad_accum_steps=${GRAD_ACCUM_STEPS:-8}  # 梯度累积步数
num_epochs=${NUM_EPOCHS:-2}  # 用户要求：2个epoch
weight_decay=${WEIGHT_DECAY:-0.01}

save_steps=${SAVE_STEPS:-500}  # 定期保存检查点
eval_strategy=${EVAL_STRATEGY:-"steps"}  # 启用评估
eval_steps=${EVAL_STEPS:-500}  # 每500步评估一次
max_eval_samples=${MAX_EVAL_SAMPLES:-2000}  # 验证集采样数量：每次评估2000个样本（约36%的数据，已添加显存优化）
