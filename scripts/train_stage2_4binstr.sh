...
if [[ "${NUM_GPUS}" -ne 4 ]]; then
  echo "[WARNING] 该脚本默认配置为 4 卡训练 (每卡约 48GB 显存)，当前检测到 ${NUM_GPUS} 卡: ${CUDA_VISIBLE_DEVICES}" >&2
  echo "          将继续使用 ${NUM_GPUS} 卡进行训练..." >&2
fi

# DeepSpeed通信配置（多卡训练）
export NCCL_P2P_DISABLE=${NCCL_P2P_DISABLE:-1}  # 启用 P2P 以提升多卡通信效率
export NCCL_IB_DISABLE=${NCCL_IB_DISABLE:-1}
export OMP_NUM_THREADS=${OMP_NUM_THREADS:-1}
export DEEPSPEED_CHECK_DATALOADER_INPUTS=0

# PyTorch 内存分配优化
# 禁用 expandable_segments 以避免内存分配错误
...
# 训练与数据配置（Stage2）
# ==============================
llm=${MODEL_PATH:-"weights/Qwen3-VL-4B-Instruct"}

# Stage1 模型路径（默认使用下载的模型）
...
batch_size=${BATCH_SIZE:-2}  # 每卡 batch size
grad_accum_steps=${GRAD_ACCUM_STEPS:-8}  # 4卡训练：有效batch = 2*4*4 = 32
num_epochs=${NUM_EPOCHS:-2}  # Stage2 训练轮数较少
...
run_name="stage2-accept-reject-4b-$(date +%Y%m%d_%H%M%S)"
...
echo ""
echo "使用 DeepSpeed 启动：1 卡 ZeRO-2（单卡训练，GPU 1）"
