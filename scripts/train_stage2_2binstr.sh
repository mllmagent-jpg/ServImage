...
if [[ "${NUM_GPUS}" -ne 4 ]]; then
  echo "[ERROR] 该脚本配置为 4 卡训练，当前检测到 ${NUM_GPUS} 卡: ${CUDA_VISIBLE_DEVICES}" >&2
  exit 1
fi
...
# 训练与数据配置（Stage2）
# ==============================
llm=${MODEL_PATH:-"weights/Qwen3-VL-2B-Instruct"}
...
# 自动提取 Stage1 LoRA 权重
# ==============================
echo "[Stage2] 检查 Stage1 LoRA 权重..."

# 检查是否已有LoRA权重目录
if [[ ! -d "${stage1_model_path}/vision_lora" ]] || [[ ! -d "${stage1_model_path}/llm_lora" ]]; then
    echo "[Stage2] 未检测到LoRA权重，开始自动提取..."
    echo ""

    # 调用Python脚本提取LoRA权重
    python scripts/extract_stage1_lora.py "${stage1_model_path}"

    extraction_status=$?
    if [[ ${extraction_status} -ne 0 ]]; then
        echo "[ERROR] LoRA权重提取失败，请检查Stage1模型是否包含有效的checkpoint" >&2
        exit 1
    fi

    echo ""
    echo "[Stage2] ✓ LoRA权重提取成功"
else
    echo "[Stage2] ✓ 检测到已存在的LoRA权重，跳过提取"
fi
...
eval_steps=${EVAL_STEPS:-500}  # 每100步评估一次
max_eval_samples=${MAX_EVAL_SAMPLES:-10000}  # 验证集采样数量：每次评估500个样本（快速评估）  
...
stage2_data_dir="data/Label_data/standard_split"
...
echo ""
echo "使用 DeepSpeed 启动：4 卡 ZeRO-2（4卡并行训练）"
